...
# ------------------------------------------------------------------
# Step 4: Inline runtime validation (remote-safe replacement for the
#         validation you ran manually on .172)
# ------------------------------------------------------------------
`$svc = Get-Service -Name `$ServiceNames -ErrorAction Stop
...
"@
    ...
    if ($text -notmatch '\[OK\] Runtime prepared and services running\.') {
