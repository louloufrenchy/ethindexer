...
function Invoke-RemoteRuntimePrepare {
    ...
    Write-Host ">>> Validating secrets configuration..."
    Invoke-RemotePS -Host $Host -Script (Get-Content "$PSScriptRoot\validation\Test-ForensicConfig.ps1" -Raw)
    Write-Host "[OK] Secrets configuration validated."

    if ($SyncSecrets) {
        Write-Host ">>> Syncing secrets to $Host ..."
        Sync-ForensicSecretsRemote -Host $Host -SourceSecretsRoot $Global:ForensicSecretsRoot
        Write-Host "[OK] Secrets synced to $Host."
    }
...
# --- SECRETS GUARD STARTS HERE ---
`$SecretsRoot = 'C:\forensic_secrets'
`$EnvPath     = Join-Path `$SecretsRoot '.env'

Write-Host "[INFO] Validating secrets at `$EnvPath"

if (-not (Test-Path `$EnvPath)) {
    throw "[CONFIG] Secrets file not found at `$EnvPath. Cannot start services."
}
...
if (`$bad.Count -gt 0) {
    throw "[CONFIG] Secrets contain placeholder values: $(`$bad -join ', '). Refusing to start services."
}

Write-Host "[OK] Secrets validated. Proceeding with runtime preparation."
# --- SECRETS GUARD ENDS HERE ---
...
`$InstallIndexerHostCandidates = @(
    ...
    (Join-Path `$SuiteRoot 'scripts\install\Install-IndexerHost.ps1'.Replace('/', '\')),
...
# ------------------------------------------------------------------
# Step 1: Run the same host-bootstrap script that fixed .172
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Step 2: Run the same service installer that fixed .172
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Step 3: Explicitly start services, same as manual recovery flow
# ------------------------------------------------------------------
...
# Verify core imports against the exact interpreter the services use
