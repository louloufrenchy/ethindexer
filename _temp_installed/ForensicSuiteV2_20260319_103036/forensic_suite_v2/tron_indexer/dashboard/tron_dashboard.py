...
# ------------------------------------------------------------
# BASE_DIR = forensic_suite_v2/tron_indexer
...
async def fetch_block_counts(conn):
    row = await conn.fetchrow("SELECT COUNT(*) AS c FROM tron_blocks")
    tx = await conn.fetchrow("SELECT COUNT(*) AS c FROM tron_transactions")
    trc20 = await conn.fetchrow("SELECT COUNT(*) AS c FROM tron_trc20_transfers")
    return row["c"], tx["c"], trc20["c"]
...
async def main():
    ...
    print("[TRON DASHBOARD] Connected to Postgres")
    time.sleep(1)

    while True:
        metrics = await fetch_metrics(conn)
        blocks, txs, trc20 = await fetch_block_counts(conn)

        clear()
        print("=== TRON Ingestion Dashboard (updates every 2s) ===")
        ...
        if metrics:
            ...
            print("No TRON metrics yet. Waiting for indexer…")

        print("")
        print("=== Table Counts ===")
        print(f"tron_blocks:          {blocks:,}")
        print(f"tron_transactions:    {txs:,}")
        print(f"tron_trc20_transfers: {trc20:,}")
