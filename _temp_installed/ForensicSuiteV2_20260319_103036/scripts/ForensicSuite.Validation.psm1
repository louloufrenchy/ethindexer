...
function Invoke-RemotePS {
    ...
    if (-not $Global:SSHKey) {
        Write-Host "[ERROR] Global:SSHKey is not set." -ForegroundColor Red
        return "SSH_ERROR"
    }

    if (-not (Test-Path $Global:SSHKey)) {
        Write-Host "[ERROR] SSH key not found: $Global:SSHKey" -ForegroundColor Red
        return "SSH_ERROR"
    }

    if ([string]::IsNullOrWhiteSpace($Script)) {
        Write-Host "[ERROR] Remote script is empty." -ForegroundColor Red
        return "SSH_ERROR"
    }
...
exit 0
    ...
    Write-Output ('_REMOTE_ERROR_ ' + `$_.Exception.Message)
    exit 1
}
"@
        ...
        Write-Host "[ERROR] Invoke-RemotePS failed for $Host : $($_.Exception.Message)" -ForegroundColor Red
        return "SSH_ERROR"
    }
}
...
function Ensure-NSSMRemote {
    ...
    $LocalNssmExe  = "F:\tools\nssm\nssm.exe"
    $RemoteNssmDir = "C:\tools\nssm"
    $RemoteNssmExe = "C:\tools\nssm\nssm.exe"
    ...
    if (-not (Test-Path $scpExe)) {
        Write-Host "[ERROR] scp.exe not found at $scpExe" -ForegroundColor Red
        return 900
    }

    if (-not (Test-Path $LocalNssmExe)) {
        Write-Host "[ERROR] NSSM binary missing on control host: $LocalNssmExe" -ForegroundColor Red
        return 901
    }

    if (-not $Global:SSHKey) {
        Write-Host "[ERROR] Global SSH key path is not set." -ForegroundColor Red
        return 901
    }
...
"@
    ...
    if (($mkdirResult | Out-String) -match "PRESENT") {
    ...
    & $scpExe `
        -i $Global:SSHKey `
        "$LocalNssmExe" `
        "forensicuser@${Host}:C:/tools/nssm/nssm.exe" `
        2>$null
...
"@
    ...
    if ((($verifyResult | Out-String).Trim()) -notmatch "^OK$") {
...
# ---------------------------------------------------------------------------
# Export module members (functions + aliases)
# ---------------------------------------------------------------------------
$modulePath = Join-Path $PSScriptRoot 'validation\ForensicSecrets.psm1'
...
if ($Host.Name -eq 'ConsoleHost') {
    ...
    Write-Host "  Invoke-BuildSuiteSafe   (clean + deterministic full build)"
    Write-Host "  Get-StatusReport        (runtime / cluster health report)"
    Write-Host "  Invoke-ForensicRelease  (full workflow deployment)"
