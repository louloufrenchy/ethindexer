...
class TronQuickNodeIndexerV2:
    def __init__(self, config):
        ...
        base_urls = tron_cfg.get("base_urls")
        if not base_urls:
            # Backward compatibility: single base_url
            rpc_base = tron_cfg["base_url"].rstrip("/")
            base_urls = [rpc_base + "/"]

        # Normalize and derive JSON + Wallet endpoints
        normalized = [u.rstrip("/") for u in base_urls]
        self.rpc_json_endpoints = tron_cfg.get("json_endpoints") or [
            u + "/jsonrpc" for u in normalized
        ]
        self.rpc_wallet_endpoints = tron_cfg.get("wallet_endpoints") or [
            u + "/wallet" for u in normalized
        ]
        ...
        # last-block tracking file
        self.last_block_file = r"C:\forensic_suite_v2\logs\tron\last_block.txt"

        print("[INIT] TRON indexer v2.0 initialized")
        print(f"[INIT] JSON endpoints: {self.rpc_json_endpoints}")
        print(f"[INIT] Wallet endpoints: {self.rpc_wallet_endpoints}")
    ...
    async def rollback_from(self, conn, rollback_to):
        ...
        await conn.execute(
            "DELETE FROM trc20_transfers WHERE block_number >= $1",
            rollback_to,
        )
        await conn.execute(
            "DELETE FROM address_tx_index WHERE block_number >= $1",
            rollback_to,
        )
        await conn.execute(
            "DELETE FROM trx_transfers WHERE block_number >= $1",
            rollback_to,
        )
        await conn.execute(
    ...
    async def ensure_monthly_partitions(self, conn, ts: datetime):
        ...
        await conn.execute(
            ...
            BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_class
                    WHERE relname = 'tron_transactions_{suffix}'
                ) THEN
                    EXECUTE $ct$
                        CREATE TABLE tron_transactions_{suffix}
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # block workers (each with its own DB connection)
            block_worker_conns = []
            block_workers = []
            ...
            # producer + metrics + queue monitor
            ...
            # close block worker DB connections
...
if __name__ == "__main__":
    ...
    print("[TRON] Loaded TRON indexer from:", __file__)
