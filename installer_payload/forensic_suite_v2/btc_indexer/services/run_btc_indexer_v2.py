...
logger = logging.getLogger("btc_indexer")
...
def log_runtime_context(cfg: Dict[str, Any]) -> None:
    btc_cfg = cfg["btc"]
    logger.info(f"[BTC] DEFAULT_CONFIG={DEFAULT_CONFIG}")
    logger.info(f"[BTC] QUICKNODE_BTC_ENDPOINT_1 present={os.getenv('QUICKNODE_BTC_ENDPOINT_1') is not None}")
    logger.info(f"[BTC] QUICKNODE_BTC_ENDPOINT_2 present={os.getenv('QUICKNODE_BTC_ENDPOINT_2') is not None}")
    logger.info(f"[BTC] endpoint_1={btc_cfg.get('endpoint_1')}")
    logger.info(f"[BTC] endpoint_2={btc_cfg.get('endpoint_2')}")
...
async def async_main() -> None:
    ...
    btc_cfg = build_runtime_config(cfg, "btc", args)
