...
function Clear-ForensicSuiteAll {
    ...
    Write-Host "=== Clear-ForensicSuiteAll (V9.3.3) ===" -ForegroundColor Cyan
...
function Repair-Workspace {
    ...
    Write-Host "`n=== Repair‑Workspace (V9.3.3 — Deterministic Recovery) ===" -ForegroundColor Cyan
    Write-Host "Repo A: $RepoA" -ForegroundColor Gray
    Write-Host "Repo B: $RepoB" -ForegroundColor Gray

    # ------------------------------------------------------------
    # 1. Clear Repo B (strict runtime‑only reset)
    # ------------------------------------------------------------
    $cleanup = Join-Path $RepoA "scripts\forensic_suite_RepoB_cleanup.ps1"
    ...
    # ------------------------------------------------------------
    # 2. Preflight (Repo A + Repo B validator)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 3. Sync‑Dev Apply (rebuild Repo B deterministically)
    # ------------------------------------------------------------
    ...
    Write-Host "`n[SUCCESS] Workspace repaired successfully (V9.3.3)." -ForegroundColor Green
    return 0
}
...
function Refresh-InstallerPayload {
    param(
    ...
    Write-Host ">>> Running Refresh‑InstallerPayload..." -ForegroundColor Yellow
    & $script
    return $LASTEXITCODE
}
...
function Invoke-BuildSuite {
    ...
    try {
		...
		& (Join-Path $RepoRoot "scripts\refresh_installer_payload.ps1") -ProjectRoot $RepoRoot
...
function Get-ForensicSuiteHelp {
    ...
    Write-Host "        FORENSIC SUITE WORKSPACE — OPERATOR HELP (V9.3.3)   " -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host " CORE WORKFLOWS (V9.3.3)    " -ForegroundColor DarkCyan
    Write-Host "----------------------------" -ForegroundColor DarkCyan
    ...
    Write-Host "    • DryRun (V9.3.3)" -ForegroundColor Gray
    Write-Host "    • Apply (atomic temp mirror → atomic swap)" -ForegroundColor Gray
    Write-Host "    • Validate (V9.3.3 strict suite scope)" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "Repair-Workspace" -ForegroundColor Yellow
    Write-Host "  Repo B cleanup → Preflight → Sync‑Dev Apply → Final validation." -ForegroundColor Gray
    Write-Host ""

    Write-Host "Refresh-InstallerPayload" -ForegroundColor Yellow
    Write-Host "  Rebuilds installer_payload using strict allow-list rules (wheel, scripts, contamination cleanup)." -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "    • Installer-payload integrity validation (V9.3.3)" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "payloadfix" -ForegroundColor Yellow
    Write-Host "  Alias for Refresh-InstallerPayload (strict payload rebuild)." -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "---------------------------" -ForegroundColor DarkCyan
    Write-Host " ALIASES (V9.3.3)          " -ForegroundColor DarkCyan
    Write-Host "---------------------------" -ForegroundColor DarkCyan
    ...
    Write-Host "repair             → Repair-Workspace" -ForegroundColor Gray
    Write-Host "safebuild          → safe-build" -ForegroundColor Gray
    Write-Host "payloadfix         → Refresh-InstallerPayload" -ForegroundColor Gray
    ...
    Write-Host "---------------------------" -ForegroundColor DarkCyan
    Write-Host " V9.3.3 EXCLUSION MODEL    " -ForegroundColor DarkCyan
    Write-Host "---------------------------" -ForegroundColor DarkCyan
    ...
    Write-Host "  • Mapping generator (V9.3.3)" -ForegroundColor Gray
    Write-Host "  • Sync‑DevTrees.DryRun (V9.3.3)" -ForegroundColor Gray
    Write-Host "  • Sync‑DevTrees.Apply (V9.3.3)" -ForegroundColor Gray
    Write-Host "  • Sync‑DevTrees.Validate (V9.3.3)" -ForegroundColor Gray
    ...
    Write-Host "  4. Final validation (V9.3.3 strict)" -ForegroundColor Gray
    Write-Host ""

    Write-Host "=============================================================" -ForegroundColor Cyan
    Write-Host " Forensic Suite V9.3.3 — Deterministic, Runtime‑Only, Atomic " -ForegroundColor Cyan
    Write-Host "=============================================================" -ForegroundColor Cyan
    Write-Host ""
}

# =====================================================================
# 9. EXPORTS, MODULE BANNER & QUICK OPTIONS
# =====================================================================

Export-ModuleMember -Function `
    Clear-ForensicSuiteAll, Test-ForensicSuiteAll, Invoke-Preflight, `
    Update-ForensicSuiteDev, Update-ForensicSuiteValidation, `
    Invoke-BuildSuiteSafe, Invoke-BuildSuite, `
    Invoke-DeployPreflight, Get-DeploySuiteSummary, `
    Invoke-DeployHost, Invoke-DeployCluster, Repair-Workspace, Refresh-InstallerPayload, Update-ForensicSuiteSecrets, `
...
Write-Host ""
Write-Host "Forensic Suite Workspace V9.3.2 Loaded." -ForegroundColor Yellow
Write-Host ""

# ---------------------------------------------------------------------
# Dynamically list exported commands
# ---------------------------------------------------------------------

$mod = $MyInvocation.MyCommand.Module
# ---------------------------------------------------------------------
# Dynamically list exported commands (robust)
# ---------------------------------------------------------------------

$exported = @()

# 1) Best case: resolve from module object
$mod = $MyInvocation.MyCommand.Module
if ($mod) {
    $exported = $mod.ExportedCommands.Keys
}

# 2) Fallback: resolve by loaded module name
if (-not $exported -or $exported.Count -eq 0) {
    $loaded = Get-Module -Name "ForensicSuite.Validation" -ErrorAction SilentlyContinue
    if ($loaded) {
        $exported = $loaded.ExportedCommands.Keys
    }
}

# 3) Final fallback: resolve by command discovery
if (-not $exported -or $exported.Count -eq 0) {
    $exported = Get-Command -Module "ForensicSuite.Validation" -ErrorAction SilentlyContinue |
...
if ($exported.Count -gt 0) {
    foreach ($cmd in $exported) { Write-Host "  $cmd" -ForegroundColor Yellow }
} else {
    Write-Host "  [NONE FOUND] Module command list unavailable at load time." -ForegroundColor DarkYellow
}
...
Write-Host "  Get-ForensicSuiteHelp (paged help overview)" -ForegroundColor White
Write-Host "  Invoke-BuildSuiteSafe   (clean + deterministic full build)" -ForegroundColor White
Write-Host "  Get-StatusReport        (runtime / cluster health report)" -ForegroundColor White
Write-Host ""
Write-Host "Convenience aliases:" -ForegroundColor White
Write-Host "cleanall " -ForegroundColor White
Write-Host "payloadfix " -ForegroundColor White
Write-Host "repair " -ForegroundColor White
Write-Host "safe-build " -ForegroundColor White
Write-Host "syncdev " -ForegroundColor White
Write-Host "validate " -ForegroundColor White
