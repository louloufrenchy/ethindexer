...
# --- NSSM runtime ---
$NssmSrc = Join-Path $RepoRoot "third_party\nssm\nssm.exe"
$NssmDst = Join-Path $PayloadRoot "tools\nssm"

if (-not (Test-Path $NssmSrc)) {
    throw "NSSM not found at $NssmSrc"
}

New-Item -ItemType Directory -Path $NssmDst -Force | Out-Null
Copy-Item $NssmSrc (Join-Path $NssmDst "nssm.exe") -Force
