...
class BtcIndexerV2:
    def __init__(self, config):
        ...
        # --- QuickNode BTC RPC (multi-endpoint rotation) ---
        rpc_cfg = config["btc_rpc"]
        self.rpc_urls = [u.rstrip("/") for u in rpc_cfg["base_urls"]]
        if not self.rpc_urls:
            raise ValueError("[BTC RPC] No BTC base_urls configured")
        self._rpc_idx = 0
    ...
    async def rpc_call(self, session, method, params):
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                last_exc = e
                print(
                    f"[BTC RPC ERROR] {method} {params} @ {url} → {e} "
                    f"(attempt {attempt}/{self.max_retries})"
                )
                await asyncio.sleep(0.5)

        print(f"[BTC RPC] Max retries exceeded for {method} (last error: {last_exc})")
        return None
